...
### Fixed（2026-04 补丁：图谱跳转与切换恢复）

- 修复图谱点击 `tag/entity` 节点时误触发“引用缺少文档标识”提示：语义节点不再走 PDF 跳转。
- 修复加载 Demo 时全局 `startOver` 清空会话缓存导致的 A→Demo→A 回切重解析问题。
- 修复文献切换中 Demo 本地态恢复缺口：Demo 与本地文献统一走 Local-First 恢复链路（高亮/摘要缓存更稳定）。
