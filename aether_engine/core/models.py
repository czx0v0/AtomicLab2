...
class Annotation(BaseModel):
    ...
    selected_text: str = Field(
        ..., description="The exact text selected from the document"
    )
    ...
    translation: Optional[str] = Field(
        None, description="Optional translation of the selected text"
    )
...
class AtomicNote(BaseModel):
    ...
    keywords: List[str] = Field(
        default_factory=list, description="List of relevant keywords"
    )
    annotations: List[Annotation] = Field(
        default_factory=list, description="Source annotations backing this note"
    )
...
class Section(BaseModel):
    ...
    atomic_notes: List[AtomicNote] = Field(
        default_factory=list, description="Atomic notes extracted from this section"
    )
...
class Document(BaseModel):
    ...
    references: List[str] = Field(
        default_factory=list, description="List of doc_ids referenced by this document"
    )
    sections: List[Section] = Field(
        default_factory=list, description="Sections contained in the document"
    )
...
class Domain(BaseModel):
    ...
    documents: List[Document] = Field(
        default_factory=list, description="List of documents within this domain"
    )
